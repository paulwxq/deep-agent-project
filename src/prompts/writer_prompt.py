...
在撰写或修订涉及第三方库/框架的技术方案时，使用 Context7 工具查阅最新官方文档：
1. 先调用 `{resolve_tool}` 确定目标库的准确 ID（如输入 "langgraph" → "/langchain-ai/langgraph"）
2. 再调用 `{query_tool}` 并传入 libraryId 和具体的 query，获取相关文档片段
3. 将检索结果作为技术细节的参考依据，确保 API 用法与当前版本一致
"""
    else:
        context7_section = ""
...
{hil_clarify_section}{context7_section}
...
任务规划：
开始工作前，必须且只能使用 `write_todos` 工具创建任务计划（严禁使用 todo_write 等变体名称），追踪进度。按工作模式分别规划：
